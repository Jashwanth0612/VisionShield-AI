...
function Comparison({ result, preview, view }) {
  ...
  if (view !== 'annotated') return <img className="result-image" src={src} alt={view === 'original' ? 'Original uploaded scene' : 'NAFNet-enhanced scene'} />
  const width = result.image_size?.width || 1
  const height = result.image_size?.height || 1
  return <div className="bbox-stage"><img className="result-image" src={result.enhanced_image || preview} alt="Enhanced scene with RT-DETR object detection overlays" />{result.detections.map((detection, index) => {
    ...
    <div className="two-col">
      <Panel eyebrow="01 / INPUT + CONTROL" title="Inference workspace">
        ...
        <div className="control-list">
          ...
          <div className="range-row"><div><b>RT-DETR confidence</b><strong>{Math.round(confidence * 100)}%</strong></div><input type="range" min=".05" max=".95" step=".01" value={confidence} onChange={(e) => setConfidence(Number(e.target.value))} aria-label="RT-DETR confidence threshold" /></div>
        </div>
        ...
        {error && <div className="error-box" role="alert"><b>Inference unavailable</b><span>{error}</span></div>}
      </Panel>

      <Panel eyebrow="02 / OUTPUT + OVERLAY" title="Detection result" action={result && <span className="success">● MEASURED</span>}>
        <div className="tabs" role="tablist" aria-label="Result views">{['original', 'enhanced', 'annotated'].map((item) => <button className={view === item ? 'active' : ''} key={item} onClick={() => setView(item)} role="tab" aria-selected={view === item}>{item}</button>)}</div>
