...
export default function VideoIntelligence() {
  ...
  const run = async () => {
    ...
    try { const response = await api.analyzeVideo({ file, enhancement, sampleFps, confidence }); setResult(response.result) }
  ...
  return <div className="page">
    ...
    <div className="two-col">
      <Panel eyebrow="01 / SEQUENCE INPUT" title="Video source">
        <Dropzone file={file} onFile={(next) => { setFile(next); setResult(null); setError('') }} accept="video/mp4,video/quicktime,video/webm,video/x-msvideo,video/mpeg" label="Drop a video here" hint="MP4, MOV, WebM, AVI, MPEG · max 100 MB · max 120 sec" />
