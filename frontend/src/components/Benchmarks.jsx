...
export default function Benchmarks() {
  ...
  const runBenchmark = async () => {
    ...
    try { await api.runImageBenchmark({ file, runs, enhancement, confidence, weather }); await load() } catch (err) { setError(err.message || 'Benchmark runtime unavailable.') }
  ...
  return <div className="page">
    ...
    <Panel eyebrow="01 / EXPLICIT BENCHMARK" title="Run a measured benchmark">
      <div className="benchmark-control"><Dropzone file={file} onFile={setFile} accept="image/*" label="Drop the benchmark image here" hint="The same image is used for each measured iteration." /><div className="benchmark-options"><label>Weather route <select value={weather} onChange={(e) => setWeather(e.target.value)}>{WEATHER_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Iterations <select value={runs} onChange={(e) => setRuns(Number(e.target.value))}><option value="1">1 run</option><option value="3">3 runs</option><option value="5">5 runs</option><option value="10">10 runs</option></select></label><label>Confidence <input type="range" min=".05" max=".95" step=".01" value={confidence} onChange={(e) => setConfidence(Number(e.target.value))} /><b>{Math.round(confidence * 100)}%</b></label><label className="inline-toggle"><span>NAFNet</span><button className={`toggle ${enhancement ? 'active' : ''}`} onClick={() => setEnhancement(!enhancement)}><i /></button></label><button className="primary" disabled={!file || loading} onClick={runBenchmark}>{loading ? 'Measuring…' : 'Run benchmark →'}</button></div></div>
    ...
    <Panel eyebrow="04 / RUN LEDGER" title="Measured benchmark history" action={modelConfigs.length > 1 && <span className="muted">{modelConfigs.length} configurations</span>}>
      {measured.length ? <div className="history-table benchmark-table"><div className="history-header"><span>Timestamp</span><span>Type</span><span>Weather route</span><span>Latency</span><span>FPS</span><span>Detections</span><span>Model config</span></div>{measured.map((run) => <div className="history-row static" key={run.run_id}><span>{new Date(run.timestamp).toLocaleString()}</span><span>{run.media_type}</span><span>{run.weather_label || run.weather_route || '—'}</span><span>{run.latency_ms} ms</span><span>{run.fps}</span><span>{run.detections}</span><span>{run.model_config}</span></div>)}</div> : <EmptyState title="No measured benchmark runs" detail="No benchmark numbers are fabricated. Run the explicit action above after model weights are configured." />}
    </Panel>
  </div>
}
